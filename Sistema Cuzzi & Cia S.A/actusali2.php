...
// end Recordset
// begin Recordset
$colmat__actuliza = '374';
if (isset($_GET['idmaterial'])) {
  $colmat__actuliza = $_GET['idmaterial'];
}
$colname__actuliza = '41710';
if (isset($_GET['idmovimiento'])) {
  $colname__actuliza = $_GET['idmovimiento'];
}
$query_actuliza = sprintf("SELECT * FROM movimientos WHERE idmovimiento = %s and idmaterial = %s", GetSQLValueString($colname__actuliza, "int"),GetSQLValueString($colmat__actuliza, "int"));
...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "frm_salidas")) {
	...
	//actulaizar salida
	$upsali = "UPDATE movimientos SET idmaterial=$material, motivo='$motivo', fecha='$fecha', idorden=$orden, idseccion=$seccion, cantidad=$cantsali, referencia='$ref', tipodoc=$tipodoc, serie=$serie, tipoope=$tipoope WHERE idmovimiento=$id";
...
<form action="<?php echo $editFormAction; ?>" method="POST" name="frm_salidas" id="frm_salidas">
  <table cellpadding="2" cellspacing="0" class="KT_tngtable">
    ...
    <tr>
      ...
      <td>
        <select name="motivo" id="motivo">
          ...
          <option value="4Conversion" <?php if (!(strcmp("4Conversion", $actuliza->Fields('motivo')))) {echo "SELECTED";} ?>>Conversi&oacute;n 
          <option value="5Devolucion" <?php if (!(strcmp("5Devolucion", $actuliza->Fields('motivo')))) {echo "SELECTED";} ?>>Devolucion
        </select>
      </td>
    </tr>
    <tr>
      ...
      <td><label for="tipodoc"></label>
        <select name="tipodoc" id="tipodoc">
          <option value="01" <?php if (!(strcmp(01, $actuliza->Fields('tipodoc')))) {echo "SELECTED";} ?>>Factura</option>
          <option value="09" <?php if (!(strcmp(09, $actuliza->Fields('tipodoc')))) {echo "SELECTED";} ?>>Guia Remisi&oacute;n</option>
          <option value="00" selected="selected" <?php if (!(strcmp(00, $actuliza->Fields('tipodoc')))) {echo "SELECTED";} ?>>Otros (Salida Materiales)</option>
        </select></td>
    </tr>
    <tr>
    ...
    <tr>
      ...
      <td><input name="referencia" type="text" id="referencia4" value="<?php echo $actuliza->Fields('referencia'); ?>" /></td>
    </tr>
    <tr>
      <td class="KT_th"><label for="fecha">Fecha:</label>
      </td>
      <td>
        <input name="fecha" id="fecha" value="<?php echo $actuliza->Fields('fecha'); ?>" wdg:mondayfirst="false" wdg:subtype="Calendar" wdg:mask="<?php echo $KT_screen_date_format; ?>" wdg:type="widget" wdg:singleclick="false" wdg:restricttomask="no" />
      </td>
    </tr>
    <tr>
      <td class="KT_th"><label for="tipoope">Tipo Operacion:</label></td>
      <td><select name="tipooper" id="tipooper">
        <option value="01" <?php if (!(strcmp(01, $actuliza->Fields('tipoope')))) {echo "SELECTED";} ?>>Venta</option>
        <option value="02" <?php if (!(strcmp(02, $actuliza->Fields('tipoope')))) {echo "SELECTED";} ?>>Compra</option>
        <option value="10" selected="selected" <?php if (!(strcmp(10, $actuliza->Fields('tipoope')))) {echo "SELECTED";} ?>>Salida a Prod.</option>
        <option value="19" <?php if (!(strcmp(19, $actuliza->Fields('tipoope')))) {echo "SELECTED";} ?>>Saldo Inicial</option>
      </select></td>
    </tr>
    <tr>
 ...
 ?>tipo_reload();
    ...
    <tr class="KT_buttons">
      <td colspan="2">
        ...
        <input type="submit" name="KT_Insert1" id="KT_Insert1" value="Actualizar" />
      </td>
    </tr>
  </table>
