...
{
t = $('#orden').val();
$.post("mi_php.php",{orden:t},function(respuesta){
	if (respuesta=="ok"){
	$("#montou").removeAttr("disabled");}
	else {
	$('#valor').val($('#montou').val());
	$("#montou").attr("disabled", true);}
	}
);
}
</script>
...
<body>
<form action="<?php echo $editFormAction; ?>" method="POST" name="form1" onSubmit="valoru();">
<table cellpadding="2" cellspacing="0" class="KT_tngtable">
  ...
  <tr class="KT_tngtable">
    ...
    <td class="KT_th">P.xMillar:</td>
  </tr>
  <tr class="KT_tngtable">
    <td>
      <select name="orden" id="orden" onChange="enviar_variables();">
        <?php
  while(!$orden->EOF){
?>
...
?>
    ...
    <td><textarea name="descrip" cols="50" id="descrip" wdg:subtype="N1DependentField" wdg:type="widget" wdg:recordset="orden" wdg:valuefield="descripcion" wdg:pkey="idorden" wdg:triggerobject="orden"></textarea>
      <input name="valor" type="hidden" id="valor"></td>
    <?php if($mone=='dolar'){ ?>	
	<td><input name="montou" type="text" id="montou" wdg:subtype="N1DependentField" wdg:type="widget" wdg:recordset="orden" wdg:valuefield="preciod" wdg:pkey="idorden" wdg:triggerobject="orden"/>
	</td>
	<?php }else{ ?>
	<td><input name="montou" type="text" id="montou" wdg:subtype="N1DependentField" wdg:type="widget" wdg:recordset="orden" wdg:valuefield="precios". wdg:pkey="idorden" wdg:triggerobject="orden"/></td>
	<?php }?>
  </tr>
  <tr class="KT_buttons">
...
<hr>
<iframe width="100%" id="the_iframe" onLoad="calcHeight();" src="verfact.php?idfac=<?php echo $idfact;?>"></iframe>
</body>
</html>
<?php
$cliente->Close();
