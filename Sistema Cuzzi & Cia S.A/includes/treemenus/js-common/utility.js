...
utility.dom.setUnselectable = function(el) {
	if (is.ie) {
		utility.dom.attachEvent(el, 'onselectstart', function() {
			// This function is called only on IE
			var tagName = window.event.srcElement.tagName.toLowerCase();
...
//	HACK, CIA: trying a new way to make unselectable work on Mozilla..
...
//		});
		...
		Array_each(inputs, function(input) {
			utility.dom.attachEvent(input, 'onfocus', function() {
				el.style.MozUserSelect = 'text';
			});
			utility.dom.attachEvent(input, 'onblur', function() {
				el.style.MozUserSelect = 'none';
			});
		});
	}
	var inputs = el.getElementsByTagName('img');
	Array_each(inputs, function(input) {
		utility.dom.attachEvent(input, (is.ie?'ondragstart':'onmousedown'), function(e) {
			if (is.mozilla) {
...
utility.window.openWindow = function(target, url, width, height) {
	...
	var winargs = "width=" + width + ",height=" + height + ",resizable=No,scrollbars=No,status=Yes,modal=yes,dependent=yes,left=" + left + ",top=" + top;
	var dialogargs = "dialogWidth: " + (width+4) + "px; dialogHeight:" + (height+51) + "px; scroll:no";

	if (window.showModalDialog && 0) {
		wndHandler = window.showModalDialog(url, window, dialogargs);
	} else {
		...
		if (wndHandler) {
			...
			if (!ctrlModalBlocker) {
				...
				ctrlModalBlocker.style.zIndex = 1;
				...
				ctrlModalBlocker.onmousedown = utility.window.focusmodal;
				ctrlModalBlocker.onfocus = utility.window.focusmodal;
