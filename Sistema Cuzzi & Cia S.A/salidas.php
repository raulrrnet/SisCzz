...
$totermi = $otermi->RecordCount();
	if($totermi>0){
	$query_mat = "SELECT tipoconsumo||'/'||categoria||'/'||materiales||'/'||marcatipo||'/'||gramajecalibre||'/'||medida||'/'||unidad as mate FROM materiales1 where idmateriales = $mat1";
	$execmat = $cnx_cuzzicia->SelectLimit($query_mat) or die($cnx_cuzzicia->ErrorMsg());
	$mat = $execmat->Fields('mate');
	$error = "Salida a Orden Liquida ".$idord."\nMaterial: ".$mat."\nFecha: ".$fecha;
	mail('mario@cuzzi.net,produccion@cuzzi.net,logistica@cuzzi.net,sistemas@cuzzi.net','Salida Material a Orden Liquidada',$error);
	}
}
